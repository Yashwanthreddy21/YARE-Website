...
export async function loadCloudSnapshot(supabase: SupabaseClient, userId: string): Promise<CloudSnapshot> {
  ...
  return {
    ...
    logs: (logsRes.data ?? []).map((row: any) => ({
      ...
      timeValue: row.time_value?.slice(0, 5) ?? undefined,
      ...
      maxSnapshot: row.max_snapshot == null ? undefined : Number(row.max_snapshot),
      notes: row.notes ?? undefined,
    })),
...
export async function pushLocalSnapshot(supabase: SupabaseClient, userId: string, snapshot: CloudSnapshot) {
  ...
  if (snapshot.tasks.length) {
    const { error } = await supabase.from('tasks').upsert(snapshot.tasks.map(t => ({
      id: t.id,
      user_id: userId,
      name: t.name,
      ...
      include_in_score: t.includeInScore,
