...
export async function loadCloudSnapshot(supabase: SupabaseClient, userId: string): Promise<CloudSnapshot> {
  const [categoriesRes, tasksRes, schedulesRes, logsRes, expensesRes, jobsRes] = await Promise.all([
  ...
  for (const result of [categoriesRes, tasksRes, schedulesRes, logsRes, expensesRes, jobsRes]) {
  ...
  return {
    ...
    logs: (logsRes.data ?? []).map((row: any) => ({
      id: row.id, taskId: row.task_id, date: row.date, completed: row.completed,
      numericValue: row.numeric_value == null ? undefined : Number(row.numeric_value),
      textValue: row.text_value ?? undefined, timeValue: row.time_value?.slice(0, 5) ?? undefined,
      durationMinutes: row.duration_minutes ?? undefined,
      targetSnapshot: row.target_snapshot == null ? undefined : Number(row.target_snapshot),
      minSnapshot: row.min_snapshot == null ? undefined : Number(row.min_snapshot),
      maxSnapshot: row.max_snapshot == null ? undefined : Number(row.max_snapshot), notes: row.notes ?? undefined,
    })),
...
export async function pushLocalSnapshot(supabase: SupabaseClient, userId: string, snapshot: CloudSnapshot) {
  ...
  if (snapshot.tasks.length) {
    const { error } = await supabase.from('tasks').upsert(snapshot.tasks.map(t => ({ id: t.id, user_id: userId, name: t.name, category_id: t.categoryId, task_type: t.taskType, icon: t.icon, theme_tag: t.themeTag ?? null, unit: t.unit ?? null, target_value: t.targetValue ?? null, min_value: t.minValue ?? null, max_value: t.maxValue ?? null, frequency_type: t.frequency, start_date: t.startDate, end_date: t.endDate ?? null, active: t.active, archived: t.archived, include_in_score: t.includeInScore, reminder_enabled: t.reminderEnabled, reminder_time: t.reminderTime ?? null, notes: t.notes ?? null, sort_order: t.sortOrder })));
    if (error) throw error;
    await supabase.from('task_schedules').delete().eq('user_id', userId);
    const schedules = snapshot.tasks.flatMap(t => t.daysOfWeek.map(day => ({ user_id: userId, task_id: t.id, day_of_week: day, frequency_type: t.frequency })));
    if (schedules.length) {
