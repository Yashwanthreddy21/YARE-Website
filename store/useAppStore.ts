...
interface AppState {
  categories: Category[]; tasks: Task[]; logs: TaskLog[]; expenses: Expense[]; jobs: JobApplication[];
  addTask: (task: Task) => void; updateTask: (id: string, patch: Partial<Task>) => void; archiveTask: (id: string, archived: boolean) => void;
  deleteTask: (id: string) => void; duplicateTask: (id: string) => void; upsertLog: (log: TaskLog) => void;
  addExpense: (expense: Expense) => void; addJob: (job: JobApplication) => void;
}

export const useAppStore = create<AppState>()(persist((set) => ({
  categories, tasks: defaultTasks, logs: [], expenses: [], jobs: [],
  ...
  duplicateTask: (id) => set((s) => { const t = s.tasks.find((x) => x.id === id); return t ? { tasks: [...s.tasks, { ...t, id: crypto.randomUUID(), name: `${t.name} Copy`, sortOrder: s.tasks.length + 1 }] } : s; }),
