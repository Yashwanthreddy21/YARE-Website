  ...
  return (
    <div>
      <header className="mb-6">
        <p className="text-sm font-medium text-slate-500">{format(now, 'EEEE, MMMM d')}</p>
      ...
      <section className="card mb-6 p-5">
        <div className="flex items-center justify-between gap-5">
          <div>
            <div className="text-sm text-slate-500">Daily completion</div>
            <div className="mt-1 text-4xl font-black tabular-nums">{score}%</div>
            <div className="mt-2 text-sm text-slate-500">{completed} complete · {Math.max(scheduled.length - completed, 0)} remaining</div>
          </div>
          <div className="relative grid h-24 w-24 shrink-0 place-items-center rounded-full" style={{ background: `conic-gradient(currentColor ${score}%, rgb(226 232 240) 0)` }} aria-label={`${score}% daily completion`}>
            <div className="grid h-20 w-20 place-items-center rounded-full bg-white text-sm font-bold dark:bg-slate-900">{score}%</div>
          </div>
        </div>
        <div className="mt-5 grid grid-cols-2 gap-3">
          <div className="rounded-xl bg-slate-50 p-3 dark:bg-slate-800">
            <Flame size={18} />
            <div className="mt-2 text-xs text-slate-500">Current streak</div>
            <div className="font-bold">{streak} {streak === 1 ? 'day' : 'days'}</div>
          </div>
          <div className="rounded-xl bg-slate-50 p-3 dark:bg-slate-800">
            <Target size={18} />
            <div className="mt-2 text-xs text-slate-500">Tasks today</div>
            <div className="font-bold">{scheduled.length}</div>
          </div>
        </div>
      </section>
